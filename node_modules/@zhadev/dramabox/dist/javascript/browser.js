...
var require_clone = __commonJS({
  "node_modules/clone/clone.js"(exports2, module2) {
    var clone = (function() {
      ...
      function clone2(parent, circular, depth, prototype3, includeNonEnumerable) {
...
var require_node_cache = __commonJS({
  "node_modules/node-cache/lib/node_cache.js"(exports2, module2) {
    (function() {
      var EventEmitter2, NodeCache2, clone, splice = [].splice, boundMethodCheck = function(instance, Constructor) {
      ...
      module2.exports = NodeCache2 = (function() {
        class NodeCache3 extends EventEmitter2 {
          constructor(options = {}) {
            super();
...
// src/Dramabox.ts
var Dramabox_exports = {};
__export(Dramabox_exports, {
  DramaboxScraper: () => DramaboxScraper,
  default: () => Dramabox_default
});
module.exports = __toCommonJS(Dramabox_exports);
...
var DramaboxScraper = class {
  constructor(config = {}) {
    this.baseUrl = "https://sapi.dramaboxdb.com";
    ...
    this.requestDelay = config.requestDelay || 1e3;
    this.cache = new import_node_cache.default({
...
${fullPem}
-----END PRIVATE KEY-----`;
      return import_crypto2.default.createPrivateKey({
        key: formattedKey,
        format: "pem"
      });
    } catch (error) {
  ...
  sign(str) {
    try {
      ...
      const sign = import_crypto2.default.createSign("RSA-SHA256");
      sign.update(str);
  ...
  isTokenValid() {
    ...
    return this.tokenCache.expiry > Date.now() + 5 * 60 * 1e3;
  }
  async generateToken() {
    ...
    const url2 = `${this.baseUrl}/drama-box/ap001/bootstrap?timestamp=${timestamp}`;
    try {
      await this.applyRateLimit();
      const response = await axios_default.post(url2, { distinctId: null }, {
        headers,
      ...
      const tokenData = {
        ...
        expiry: creationTime + 24 * 60 * 60 * 1e3
      };
  ...
  buildHeaders(tokenData, timestamp, payload = {}) {
  ...
  async request(endpoint, payload = {}, isWebfic = false, method = "POST", attempt = 0) {
    try {
      await this.applyRateLimit();
      const timestamp = Date.now();
      let url2;
      ...
      if (isWebfic) {
        ...
        url2 = `${this.baseUrl}${endpoint}?timestamp=${timestamp}`;
  ...
  async getChapters(bookId) {
    try {
      ...
      const chapters = chapterList.map((chapter) => {
        const cdn = chapter.cdnList?.find((c) => c.isDefault === 1);
  ...
  async getStreamUrl(bookId, episode) {
    try {
      ...
      const url2 = "https://regexd.com/base.php";
      const headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36",
        "X-Requested-With": "XMLHttpRequest",
        Referer: `${url2}?bookId=${bookId}`
      };
  ...
  async batchDownload(bookId) {
    try {
      ...
      const finalResult = Array.from(uniqueMap.values()).sort((a, b) => (a.chapterIndex || 0) - (b.chapterIndex || 0)).map((ch) => {
        let cdn = ch.cdnList?.find((c) => c.isDefault === 1) || ch.cdnList?.[0];
        let videoPath = "N/A";
        if (cdn?.videoPathList) {
          const preferred = cdn.videoPathList.find((v) => v.isDefault === 1) || cdn.videoPathList.find((v) => v.quality === 1080) || cdn.videoPathList.find((v) => v.quality === 720) || cdn.videoPathList[0];
  ...
  async getEpisodeDetails(bookId, episodeIndex) {
    try {
      ...
      const stream4 = await this.getStreamUrl(bookId, episodeIndex);
      const responseData = {
        bookId,
        episode: {
...
var Dramabox_default = DramaboxScraper;
if (typeof window !== "undefined") {
  window.DramaboxScraper = DramaboxScraper;
}
