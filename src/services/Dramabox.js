...
export default class Dramabox {
  ...
  async generateNewToken(timestamp = Date.now(), attempt = 0) {
    ...
    try {
      console.log(
        `[Token] Generating new token (attempt ${attempt + 1}/${CONFIG.MAX_RETRIES + 1
        })...`
      );
  ...
  async request(
    ...
    try {
      ...
      if (attempt < CONFIG.MAX_RETRIES && isRetryableError(error)) {
        const retryDelay = getRetryDelay(attempt);
        console.log(
          `[Request] ⚠️ ${formatError(error)} - Retry ${attempt + 1}/${CONFIG.MAX_RETRIES
          } in ${retryDelay}ms...`
        );
    ...
    for (let attempt = 0; attempt <= CONFIG.MAX_RETRIES; attempt++) {
      try {
        ...
        if (attempt < CONFIG.MAX_RETRIES && isRetryableError(error)) {
          const retryDelay = getRetryDelay(attempt);
          console.log(
            `[Stream] ⚠️ ${formatError(error)} - Retry ${attempt + 1}/${CONFIG.MAX_RETRIES
            }...`
          );
          await delay(retryDelay);
          continue;
